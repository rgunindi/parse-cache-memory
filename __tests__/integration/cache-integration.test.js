  ...
  describe("Complex Query Scenarios", () => {
    it("should handle complex queries with multiple conditions", async () => {
      ...
      if (firstResult.length > 0) {
        ...
        expect(firstResult[0].get("field1")).toBe(
          secondResult[0].get("field1"),
        );
      }
    });
    ...
    it("should handle aggregate queries", async () => {
      ...
      const firstResult = await query.aggregateCache(pipeline, {
        useMasterKey: true,
      });
      const secondResult = await query.aggregateCache(pipeline, {
        useMasterKey: true,
      });
